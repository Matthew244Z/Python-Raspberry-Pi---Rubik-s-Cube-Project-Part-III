...
# from one coordinate, returns a list with 25 (or MATRIX) coordinates
...
def oneDimensionalList(oneDcoord):
    ...
    for i in range(SIDE_COLOR_POINTS):
        ...
        if i == 0: newPoint = oneDcoord - _OFF*2
        elif i == 1: newPoint = oneDcoord - _OFF
        elif i == 2: newPoint = oneDcoord
        elif i == 3 : newPoint = oneDcoord + _OFF
        else: newPoint = oneDcoord + _OFF*2

        oneDList.append(newPoint)

    return oneDList

# gets two list of size n
# gets list x and list y and returns list x, y of size n^2
...
def main():
    ...
    #     imageRectangle = cv2.circle(image, coordinateList[i], (10*i), (0, 0, 0), (2*i))
    #     cv2.imwrite("circleImage.png", imageRectangle)
    ...
    for i in range(CUBE_SQUARES):
        ...
        file.write(f'Coordinate:{pointList[0]},{pointList[1]}\n')
        ...
        for l in range(MATRIX):
            file.write(f'[{mainList[l]}] ')
        ...
        for j in range(CUBE_SQUARES):
            file.write(f'[{RGBList[j][0]}, {RGBList[j][1]}, {RGBList[j][2]}]\n')
